	...
	factory: (require) => {
		...
		*/
		function splitPathSegments(text) {
			const PATTERN = /* @__PURE__ */ new RegExp("(https?:\\/\\/[^\\s，。；）】”'\"<>]+|(?:(?:~/)|(?:/(?:Users|home|tmp|var|opt|etc|private|data|System)))[^\\s，。；）】”'\"<>]*|[A-Za-z0-9_\\-./]+\\.(?:png|jpeg|jpg|gif|webp|svg|bmp|ico|pdf|docx|doc|rtf|xlsx|xls|csv|css|java|kt|tsx|ts|json|jsx|js|py|go|rs|swift|bash|sh|sql|hpp|html|htm|markdown|md|txt|ya?ml|xml|zip|tar|gz|rar|7z|cpp|c)(?![A-Za-z0-9_]))", "g");
			const out = [];
			let last = 0;
		...
		function buildChip(value, isUrl) {
			...
			if (isUrl) {
				...
				el.addEventListener("click", (ev) => {
					ev.stopPropagation();
					(async () => {
						const tryOpen = async (abs) => {
							try {
								return (await fetch("/dsh-concise/api/open", {
									method: "POST",
						...
						const preAbs = el.dataset.dccAbs;
						if (preAbs && await tryOpen(preAbs)) {
							mark(el, "dcc-opened");
							return;
						}
						let candidates = [];
						try {
							const sid = globalThis.__dshConciseSid ?? "";
						...
						for (const cwd of candidates) if (await tryOpen(resolveAbsolute(path, cwd, ""))) {
							mark(el, "dcc-opened");
							return;
						}
						try {
							...
							if (res.ok) {
								const abs = (await res.json()).resolved?.[path];
		...
		*/
		...
		const ConciseButton = ({ t, sessionId }) => {
			...
			react.default.useEffect(() => {
				let disposed = false;
				globalThis.__dshConciseSid = sessionId ?? "";
		...
		*/
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const renderDeliverables = async (bq) => {
					...
					try {
						...
						let files = collectDeliverables(container ? blockAwareText(container) : bq.textContent ?? "", bq.textContent ?? "", 8);
						let resolvedAbs = {};
						if (files.length > 0) try {
							...
							if (res.ok) {
								const j = await res.json();
								if (Array.isArray(j.existing)) files = j.existing.filter((x) => typeof x === "string");
								if (j.resolved && typeof j.resolved === "object" && !Array.isArray(j.resolved)) resolvedAbs = j.resolved;
							}
						} catch {}
					...
					for (const bq of Array.from(document.querySelectorAll("blockquote"))) {
						...
						if (sig !== st.sig) {
							...
							state.timer = window.setTimeout(() => {
								...
								enhanceCard(card);
								const attempt = (left) => {
									renderDeliverables(card).then((rendered) => {
										state.enhanced = rendered;
										if (!rendered && left > 0 && card.isConnected) window.setTimeout(() => {
											if (card.isConnected) attempt(left - 1);
										}, 2e3);
									});
								};
								attempt(2);
