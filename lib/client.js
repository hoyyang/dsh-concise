	...
	factory: (require) => {
		...
		*/
		function splitPathSegments(text) {
			const PATTERN = new RegExp("(https?:\\/\\/[^\\s，。；：）（】【、”“’‘《》「」『』？！…—·'\"<>]+|(?:(?:~/)|(?:/(?:Users|home|tmp|var|opt|etc|private|data|System)))[\\p{L}\\p{N}\\-._/~@+()]*|[A-Za-z0-9_\\-./]+\\.(?:" + FILE_EXT_RE_SOURCE + ")(?![A-Za-z0-9_])" + "|(?:[\\p{L}\\p{N}\\-._]+\\/)+[\\p{L}\\p{N}\\-._]+\\.(?:" + FILE_EXT_RE_SOURCE + ")(?![A-Za-z0-9_]))", "gu");
			const out = [];
			let last = 0;
			for (const m of text.matchAll(PATTERN)) {
				...
				out.push({
					type: /^https?:/i.test(anchored) ? "url" : "file",
					value: anchored
				});
				if (anchorTail) out.push({
					type: "text",
					value: anchorTail
				});
