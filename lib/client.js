	...
	factory: (require) => {
		...
		*/
		function splitPathSegments(text) {
			const PATTERN = /* @__PURE__ */ new RegExp("(https?:\\/\\/[^\\s，。；）】”'\"<>]+|(?:~|/(?:Users|home|tmp|var|opt|etc|private|data|System))[^\\s，。；）】”'\"<>]*|[A-Za-z0-9_\\-./]+\\.(?:png|jpe?g|gif|webp|svg|bmp|ico|pdf|docx?|rtf|xlsx?|csv|java|kt|tsx?|jsx?|py|go|rs|c|cpp|h|hpp|sh|bash|sql|swift|markdown|md|txt|json|ya?ml|xml|html?|css|zip|tar|gz|rar|7z))", "g");
			const out = [];
			let last = 0;
		...
		*/
		async function openViaHost(path) {
			try {
				...
				if (!cachedApps) {
					...
					cachedApps = (Array.isArray(wrapped?.apps) ? wrapped.apps : []).map((x) => typeof x === "string" ? x : x?.id).filter((x) => typeof x === "string" && x.length > 0);
				}
				const apps = cachedApps;
				if (!apps || apps.length === 0) return false;
				const has = (id) => apps.includes(id);
				const info = detectPathKind(path);
				const appId = (info.kind === "code" || info.kind === "markdown" ? [
					"cursor",
					"vscode",
					"finder",
					"explorer",
					...
					"cursor",
					"vscode"
				]).find((id) => has(id)) ?? apps[0];
				const launch = async (target) => {
					return (await fetch(new URL("/open-in-app/open", base), {
						method: "POST",
			...
			if (isUrl) {
				...
				el.addEventListener("click", (ev) => {
					...
					(async () => {
						...
						const info = detectPathKind(path);
						const scheme = info.kind === "code" || info.kind === "markdown" || info.kind === "image" || info.kind === "file" ? ideSchemeUrl(apps, abs) : null;
						if (scheme) {
							window.open(scheme, "_blank");
							mark(el, "dcc-opened");
							return;
						}
						if (await openViaHost(abs)) {
							mark(el, "dcc-opened");
							return;
						}
						try {
		...
		function ideSchemeUrl(apps, absPath) {
			const ide = [
				"cursor",
				"vscode",
				"windsurf",
				"zed"
			].find((id) => apps.includes(id));
			...
			ctx.effect(() => {
				...
				const scan = () => {
					...
					for (const bq of Array.from(document.querySelectorAll("blockquote"))) {
						...
						if (hit) for (const a of Array.from(bq.querySelectorAll("a[href]"))) {
