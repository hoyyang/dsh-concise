...
export const CONCISE_STYLE_TEXT = [
    ...
    "- Presentation artifacts between tool calls are NOT narration: a design card, decision card, comparison, or any message the user must read to act or choose (e.g. a 设计卡 issued right before an ask_user_question panel) MUST open with the digest block even though more tool steps follow - the intermediate exemption covers narration only, never user-facing artifacts. A bare panel call with no text does not satisfy this; the artifact text right before the panel carries the digest.",
...
/**
 * v0.11.2：每步请求尾部追加的摘要契约提醒（agent/pre-step 瞬态注入，不落盘不进转写）。
 * 动机（实证）：措辞迭代到 0.11.1 后「审查报告型（标题+表格）」仍漏卡（autofill 打点会话 15:06 轮，
 * 全量措辞在场 0 警告）——契约在 141K prompt 头部，生成时模型注意力在对话尾部，纯 system prompt
 * 通道存在显著性上限。本提醒拼接在每步请求的【最后一条消息】= 生成前最后一个可读位置。
 */
const DIGEST_STEP_REMINDER = 'Concise digest contract (applies to THIS turn): the user-facing final reply that ends this turn must BEGIN with the digest blockquote "> **摘要：** ..." exactly as defined in the Concise output style section of the system prompt — before any heading, table, or body text. The intermediate exemption covers tool-loop narration only - never user-facing artifacts: a design card or decision message the user reads before choosing (e.g. issued right before an ask_user_question panel) must open with the digest block. Task-completion reports, monitoring/progress confirmations (【进度】/账目/确认无误/明白), delivery talk-tracks (「图已生成…」「交付如下」), and replies ending in a user-facing question are NOT exempt.';
...
function openWithDefaultApp(path) {
    ...
    if (process.platform === 'darwin')
        spawn('open', [path], opts).unref();
    else if (process.platform === 'win32')
        spawn('cmd', ['/c', 'start', '', path], { ...opts, windowsVerbatimArguments: true }).unref();
    else
        spawn('xdg-open', [path], opts).unref();
}
/** 校验 open 目标：绝对路径 + 存在。非法返回 null。 */
function validateOpenTarget(path) {
 ...
 */
export function digestFinding(session) {
...
export function isDigestMissing(session) {
...
let zstdDecompressSyncFn;
async function zstdFn() {
    if (zstdDecompressSyncFn !== undefined)
    ...
    try {
        ...
        zstdDecompressSyncFn = null;
    }
    return zstdDecompressSyncFn;
}
/** 从会话转写首行 JSON 解出 cwd；文件超 8MB 跳过（兜底路径，不做增量索引）。 */
function parseCwdFromSessionFile(file, decompress) {
    try {
        if (statSync(file).size > 8 * 1024 * 1024)
            return null;
        const buf = decompress(readFileSync(file));
        const nl = buf.indexOf(10);
        const first = JSON.parse(buf.subarray(0, nl > 0 ? nl : buf.length).toString('utf8'));
        const cwd = first?.cwd;
...
let recentSessionCwdsCache = null;
/** 60s TTL：recentSessionCwds 需解压转写（数 MB），verify/cwd 高频调用不得反复解。 */
async function recentSessionCwdsCached(count) {
    if (recentSessionCwdsCache && Date.now() - recentSessionCwdsCache.at < 60_000)
        return recentSessionCwdsCache.cwds;
    const cwds = await recentSessionCwdsUncached(count);
    recentSessionCwdsCache = { at: Date.now(), cwds };
    return cwds;
}
async function recentSessionCwdsUncached(count) {
    try {
...
function findInDir(dir, base, budget, depth) {
    if (depth > 4 || budget.n <= 0)
        return null;
    let entries = [];
    try {
 ...
 */
async function resolveExistingPath(candidates, p) {
    if (p.startsWith('/'))
...
function sessionIdLooksSafe(sessionId) {
...
export function apply(ctx, config = {}) {
    ...
    // v0.11.2：agent/pre-step 瀑布中间件——每步 LLM 请求的【最后一条消息】后追加瞬态摘要契约提醒。
    // 先例：dsh-time-context / dsh-agent-instructions 同通道同拼接法（请求级瞬态，转写零残留，
    // 实测 time-context 在 5389 事件转写中 0 命中）。prepend:true = 本 handler 在瀑布中最先执行、
    // 其 next() 返回后再追加 → 本提醒位于其余中间件产物之后 = 请求最末位（生成前最后可读位置）。
    if (ctx.on) {
        ctx.effect(() => ctx.on('agent/pre-step', async (payload, next) => {
            const decision = await next();
            if (!decision || typeof decision !== 'object' || decision.kind === 'reject')
                return decision;
            ...
            // 主会话 id 形如 session-<uuid>；子代理是裸 UUID 独立会话（dsh-agent-billing 实证），
            // 其输出不进主会话消息流，不下发摘要契约（避免噪声与误导）。
            if (typeof rawId !== 'string' || !rawId.startsWith('session-'))
                return decision;
            if (!isEnabled(sessionIdOf({ agent: payload.agent })))
                return decision;
            log.info?.('[dsh-concise] digest step reminder attached (session ' + rawId.slice(0, 24) + '…, step ' + String(payload.step ?? '?') + ')');
            return {
                ...decision,
                messages: [...messages, {
                        id: randomUUID(),
                        role: 'user',
                        content: [{ type: 'text', text: DIGEST_STEP_REMINDER }],
                        source: { kind: 'plugin', plugin: name, form: 'digest-reminder' },
                    }],
            };
        }, { prepend: true }), 'dsh-concise: pre-step digest reminder');
    }
    ...
    function missAlert(sid, finding, kind) {
        if (finding.verdict !== 'missing' && finding.verdict !== 'misplaced') {
            if (sid !== null)
                missStreak.delete(sid);
            return null;
        }
        ...
        if (sid !== null)
            missStreak.set(sid, { sig, n });
        const opener = finding.opener ? '「' + finding.opener + '」' : '';
        const core = finding.verdict === 'misplaced'
            ? 'your PREVIOUS user-facing text buried its 摘要 digest block mid-reply' + (opener ? ' after opening with ' + opener : '') + ' — a digest the user sees mid-text is not the contract; it must BE the first line'
            : opener
                ? 'your PREVIOUS user-facing text (final reply or a presentation artifact such as a design card) opened with ' + opener + ' and has NO 摘要 digest card'
                : 'your PREVIOUS user-facing text has NO 摘要 digest card';
        ...
        const dispose = scoped.webServer.register({
            ...
            handler: async (req, res) => {
                ...
                if (path === '/verify-paths' && method === 'POST') {
                    ...
                    const paths = raw.filter((x) => typeof x === 'string' && x.length > 0 && x.length <= 1024);
                    const sid = sidFrom({});
                    ...
                    if (sid && !cwdBySession.has(sid) && /^session-[A-Za-z0-9-]{1,120}$/.test(sid)) {
                        ...
                        if (decompress) {
                            const base = process.env.DSH_HOME || join(homedir(), '.dsh');
                            for (const grp of (() => { try {
                                return readdirSync(join(base, 'sessions'));
                            }
                            catch {
                                return [];
                            } })()) {
                    ...
                    for (const v of cwdBySession.values())
                        if (v)
                            candidates.add(v);
                    if (lastKnownCwd)
                        candidates.add(lastKnownCwd);
                    for (const d of await recentSessionCwdsCached(8))
                        candidates.add(d);
