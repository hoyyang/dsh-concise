...
function findInDir(dir, base, budget, depth) {
    ...
    for (const name of entries) {
        ...
        if (isDir && !WALK_SKIP_DIRS.has(name) && !name.startsWith('.')) {
            if (findInDir(full, base, budget, depth + 1))
                return true;
        }
    }
    return false;
...
async function pathExistsAnywhere(candidates, p) {
    if (p.startsWith('/'))
        return existsSync(p);
    if (p.startsWith('~'))
        return existsSync(join(homedir(), p.slice(1)));
                ...
                if (path === '/verify-paths' && method === 'POST') {
                    ...
                    for (const p of paths) {
                        if (seen.has(p))
                            continue;
                        seen.add(p);
                        if (await pathExistsAnywhere(candidates, p))
                            existing.push(p);
                    }
                    res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' });
