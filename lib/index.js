...
function appendWarning(assembled, text) {
    if (typeof assembled?.system === 'string')
        return { ...assembled, system: assembled.system + '\n\n' + text };
    if (typeof assembled?.developer === 'string')
        return { ...assembled, developer: assembled.developer + '\n\n' + text };
    return assembled;
}
...
export function apply(ctx, config = {}) {
    ...
    if (typeof ctx.on === 'function') {
        ctx.effect(() => {
            const offs = [];
            try {
                offs.push(ctx.on('system-prompt/assemble', async (...args) => {
                    const next = args[args.length - 1];
                    const assembled = await next();
                    try {
                        const context = (args.length >= 2 ? args[args.length - 2] : undefined);
