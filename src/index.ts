...
interface HostContext {
  ...
  on?: (event: string, handler: (payload: PreStepPayload, next: () => Promise<PreStepDecision>) => Promise<PreStepDecision>, opts?: { prepend?: boolean }) => () => void
...
export function apply(ctx: HostContext, config: ConfigType = {}): void {
  ...
  if (ctx.on) {
    ctx.effect(() => ctx.on!('agent/pre-step', async (payload, next) => {
      ...
      if (typeof rawId !== 'string' || !rawId.startsWith('session-')) return decision
      if (!isEnabled(sessionIdOf({ agent: payload.agent }))) return decision
